...
テストメソッドの文 (statement) 数が多すぎるスメル。長いテストは理解しにくく、複数の関心事をテストしている可能性がある。
...
テストメソッドの文の数が閾値を超える場合に検出（デフォルト30）。コメントや空行、複数行に渡る式は数えない。複合文 (`if`/`for`/`try` など) もそれ自体を1つとして数え、ブロック (`{}`) を二重カウントすることはない。
